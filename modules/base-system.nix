...
with lib; {
  ...
  config = mkIf config.tabletop.baseSystem.enable {
    # Allow unfree packages
    nixpkgs.config.allowUnfree = true;

    # System locale and timezone
    time.timeZone = config.tabletop.baseSystem.timeZone;
    i18n.defaultLocale = config.tabletop.baseSystem.locale;
    ...
    environment.systemPackages =
      config.tabletop.baseSystem.packages.essential
      ++ config.tabletop.baseSystem.packages.development
      ++ config.tabletop.baseSystem.packages.utilities;
